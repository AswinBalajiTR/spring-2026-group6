...
SERIES_MAP = {
    ...
    "real_usd_index":       "RTWEXBGS",         # Real USD Broad Trade Weighted Index (cnn_lstm)
    "usd_eur":              "DEXUSEU",          # USD/EUR Exchange Rate (daily)
    "volatility_index":     "OVXCLS",           # CBOE Crude Oil Volatility Index (daily)
    "ind_production_overall": "INDPRO",         # Industrial Production Index (cnn_lstm)
    "ind_prod_drill":        "IPN213111S",      # Industrial Production: Drilling Oil & Gas (cnn_lstm)
    "cap_util":              "TCU",             # Total Capacity Utilization (cnn_lstm)
    "cap_util_oil_gas":      "CAPUTLG211S",     # Capacity Utilization: Oil & Gas (cnn_lstm)
    "natgas_electric":       "GASDESW",         # Natural Gas to Electric Power (cnn_lstm)
    "cpi":                   "CPIAUCSL",        # CPI All Items (cnn_lstm)
    "ppi":                   "PPIENG",          # PPI Energy (cnn_lstm)
    "ppidp":                "WPU0561",         # PPI Crude Petroleum (cnn_lstm)
    "pce":                   "PCEPI",           # PCE Price Index (cnn_lstm)
}
