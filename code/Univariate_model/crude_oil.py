...
eda.plot_series(s, name="WTI")
eda.stationarity_tests(s, name="WTI")

s_diff = eda.take_difference(s, periods=1)
eda.plot_series(s_diff, name="WTI_diff")
eda.stationarity_tests(s_diff, name="WTI_diff")
eda.plot_acf_pacf(s_diff, name="WTI_diff", lags=40)
eda.seasonality_test(s_diff, name="WTI_diff", period=12)
eda.decompose(s_diff, name="WTI_diff", period=12, model="additive")
...
ax.set_title("WTI – ARIMA Forecasts vs Actual")
ax.set_xlabel("Date")
ax.set_ylabel("Price")
ax.legend()
fig.tight_layout()
