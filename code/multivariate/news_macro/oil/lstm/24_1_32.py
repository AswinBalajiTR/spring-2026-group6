...
def main():
    ...
    best_params = m.tune_lstm(train_X, train_Y, n_trials=N_TRIALS,  # tune_lstm
                              epochs=OPTUNA_EPOCHS, batch_size=BATCH_SIZE,
                              val_split=VAL_SPLIT, patience=PATIENCE)
    model = m.lstm(train_X, train_Y, epochs=EPOCHS, batch_size=BATCH_SIZE,  # lstm
                   val_split=VAL_SPLIT, patience=PATIENCE, **best_params)
    model.save(os.path.join(results_dir, "lstm_model.keras"))  # renamed
    # ── Predict ───────────────────────────────────────────────────
