...
def main():
    ...
    df.drop(['real_usd_index','ppidp','pce','ppidp','ppi','cap_util','ind_prod_drill','natgas_electric','gasoline_price','usd_eur'],errors='ignore',inplace=True,axis=1)
    print(df.head())

    eda = EDA(results_dir, TARGET, INPUT_WINDOW)
    eda.check_multicollinearity(df,top_n=20,corr_threshold=0.3,vif_threshold=10)

    # In main, normalize BEFORE split_time_series
